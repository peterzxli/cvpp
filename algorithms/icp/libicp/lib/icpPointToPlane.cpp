...
static inline MatICP lssolvesvd(MatICP &A, MatICP &b)
{
  int i;
  MatICP U, S, V;

  A.svd(U, S, V);
  MatICP Uc=U.getMat(0, 0, A.m-1, A.n-1); // compact U
  MatICP Uctb=~Uc*b;
  for(i=0; i<Uctb.m; ++i)
...
double IcpPointToPlane::fitStep (double *T,const int32_t T_num,MatICP &R,MatICP &t,const std::vector<int32_t> &active) {

  int i;
  int nact = (int)active.size();

  // init MatICP for point correspondences
  MatICP p_m(nact,dim); // model
  MatICP p_t(nact,dim); // template
  
  // dimensionality 2
  if (dim==2) {

    // extract MatICP and translation vector
...
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,A,b,r00,r01,r10,r11,t0,t1) // schedule (dynamic,2)
    ...
    MatICP b_=lssolvesvd(A, b);
#endif

    // rotation MatICP
    MatICP R_ = MatICP::eye(2);
    R_.val[0][1] = -b_.val[0][0];
    R_.val[1][0] = +b_.val[0][0];

    // orthonormalized rotation MatICP
    MatICP U,W,V;
    R_.svd(U,W,V);
    R_ = U*~V;  

    // fix improper MatICP problem
    if (R_.det()<0){
      MatICP B = MatICP::eye(dim);
    ...
    R = R_*R;
    t = R_*t+t_;
    return max((R_-MatICP::eye(2)).l2norm(),t_.l2norm());
...
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,A,b,r00,r01,r02,r10,r11,r12,r20,r21,r22,t0,t1,t2) // schedule (dynamic,2)
    ...
    R = R_*R;
    t = R_*t+t_;
    return max((R_-MatICP::eye(3)).l2norm(),t_.l2norm());
  }
...
std::vector<int32_t> IcpPointToPlane::getInliers (double *T,const int32_t T_num,const MatICP &R,const MatICP &t,const double indist) {
...
void IcpPointToPlane::computeNormal (const kdtree::KDTreeResultVector &neighbors,double *M_normal,const double flatness) {
  ...
  if (dim==2) {
    ...
    MatICP Q = P - MatICP::ones(neighbors.size(),1)*mu;
