...
double IcpPointToPoint::fitStep (double *T,const int32_t T_num,MatICP &R,MatICP &t,const std::vector<int32_t> &active) {
...
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,r00,r01,r02,r10,r11,r12,r20,r21,r22,t0,t1,t2) reduction(+:mum0,mum1,mum2, mut0,mut1,mut2) // schedule (dynamic,2)
  ...
  MatICP q_m = p_m - MatICP::ones(active.size(),1)*mu_m;
  MatICP q_t = p_t - MatICP::ones(active.size(),1)*mu_t;

  // compute relative rotation MatICP R and translation vector t
  MatICP H = ~q_t*q_m;
  MatICP U,W,V;
  H.svd(U,W,V);
  MatICP R_ = V*~U;
  ...
  if (dim==2) return max((R_-MatICP::eye(2)).l2norm(),t_.l2norm());
  else        return max((R_-MatICP::eye(3)).l2norm(),t_.l2norm());
}

std::vector<int32_t> IcpPointToPoint::getInliers (double *T,const int32_t T_num,const MatICP &R,const MatICP &t,const double indist) {
