...
MatICP::MatICP (const int32_t m_,const int32_t n_) {
  allocateMemory(m_,n_);
}

MatICP::MatICP (const int32_t m_,const int32_t n_,const FLOAT* val_) {
  allocateMemory(m_,n_);
  int32_t k=0;
...
MatICP& MatICP::operator= (const MatICP &M) {
  if (this!=&M) {
    if (M.m!=m || M.n!=n) {
      releaseMemory();
      allocateMemory(M.m,M.n);
...
void MatICP::getData(FLOAT* val_,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  int32_t k=0;
...
MatICP MatICP::getMat(int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  if (i1<0 || i2>=m || j1<0 || j2>=n || i2<i1 || j2<j1) {
    cerr << "ERROR: Cannot get subMatICP [" << i1 << ".." << i2 <<
        "] x [" << j1 << ".." << j2 << "]" <<
        " of a (" << m << "x" << n << ") MatICP." << endl;
    exit(0);
  }
  MatICP M(i2-i1+1,j2-j1+1);
  for (int32_t i=0; i<M.m; i++)
...
void MatICP::setMat(const MatICP &M,const int32_t i1,const int32_t j1) {
  if (i1<0 || j1<0 || i1+M.m>m || j1+M.n>n) {
    cerr << "ERROR: Cannot set subMatICP [" << i1 << ".." << i1+M.m-1 <<
        "] x [" << j1 << ".." << j1+M.n-1 << "]" <<
        " of a (" << m << "x" << n << ") MatICP." << endl;
    exit(0);
  }
...
void MatICP::setVal(FLOAT s,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  if (i2<i1 || j2<j1) {
...
void MatICP::setDiag(FLOAT s,int32_t i1,int32_t i2) {
...
MatICP MatICP::extractCols (vector<int> idx) {
  MatICP M(m,idx.size());
  for (int32_t j=0; j<M.n; j++)
    if (idx[j]<n)
...
MatICP MatICP::eye (const int32_t m) {
  MatICP M(m,m);
  for (int32_t i=0; i<m; i++)
    M.val[i][i] = 1;
  return M;
...
MatICP MatICP::ones (const int32_t m,const int32_t n) {
  MatICP M(m,n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
      M.val[i][j] = 1;
...
MatICP MatICP::diag (const MatICP &M) {
  if (M.m>1 && M.n==1) {
    MatICP D(M.m,M.m);
    for (int32_t i=0; i<M.m; i++)
      D.val[i][i] = M.val[i][0];
  ...
  cout << "ERROR: Trying to create diagonal MatICP from vector of size (" << M.m << "x" << M.n << ")" << endl;
  exit(0);
}

MatICP MatICP::reshape(const MatICP &M,int32_t m_,int32_t n_) {
  if (M.m*M.n != m_*n_) {
    cerr << "ERROR: Trying to reshape a MatICP of size (" << M.m << "x" << M.n <<
...
MatICP MatICP::rotMatX (const FLOAT &angle) {
  FLOAT s = sin(angle);
  FLOAT c = cos(angle);
  MatICP R(3,3);
  R.val[0][0] = +1;
...
MatICP MatICP::rotMatY (const FLOAT &angle) {
  FLOAT s = sin(angle);
  FLOAT c = cos(angle);
  MatICP R(3,3);
  R.val[0][0] = +c;
...
MatICP MatICP::rotMatZ (const FLOAT &angle) {
  FLOAT s = sin(angle);
  FLOAT c = cos(angle);
  MatICP R(3,3);
  R.val[0][0] = +c;
...
MatICP MatICP::operator* (const FLOAT &s) {
  MatICP C(m,n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
      C.val[i][j] = val[i][j]*s;
...
MatICP MatICP::cross (const MatICP &a, const MatICP &b) {
...
MatICP MatICP::inv (const MatICP &M) {
  if (M.m!=M.n) {
    cerr << "ERROR: Trying to invert MatICP of size (" << M.m << "x" << M.n << ")" << endl;
    exit(0);
  }
  MatICP A(M);
  MatICP B = eye(M.m);
...
bool MatICP::inv () {
  if (m!=n) {
    cerr << "ERROR: Trying to invert MatICP of size (" << m << "x" << n << ")" << endl;
    exit(0);
  }
  MatICP A(*this);
  eye();
...
FLOAT MatICP::det () {
  ...
  if (m != n) {
    cerr << "ERROR: Trying to compute determinant of a MatICP of size (" << m << "x" << n << ")" << endl;
    exit(0);
  }
...
bool MatICP::solve (const MatICP &M, FLOAT eps) {
...
// Given a MatICP a[1..n][1..n], this routine replaces it by the LU decomposition of a rowwise
...
bool MatICP::lu(int32_t *idx, FLOAT &d, FLOAT eps) {
  
  if (m != n) {
    cerr << "ERROR: Trying to LU decompose a MatICP of size (" << m << "x" << n << ")" << endl;
    exit(0);
  }
...
// Given a MatICP M/A[1..m][1..n], this routine computes its singular value decomposition, M/A =
// U·W·V T. TheMatICP U replaces a on output. The diagonal MatICP of singular values W is output
// as a vector w[1..n]. TheMatICP V (not the transpose V T ) is output as v[1..n][1..n].
void MatICP::svd(MatICP &U2,MatICP &W,MatICP &V) {

  MatICP U = MatICP(*this);
  U2 = MatICP(m,m);
  V  = MatICP(n,n);
  ...
  W = MatICP(min(m,n),1,w);
  
  // extract mxm subMatICP U
...
ostream& operator<< (ostream& out,const MatICP& M) {
  if (M.m==0 || M.n==0) {
    out << "[empty MatICP]";
  } else {
    char buffer[1024];
...
void MatICP::allocateMemory (const int32_t m_,const int32_t n_) {
  m = abs(m_); n = abs(n_);
  if (m==0 || n==0) {
    val = 0;
    return;
...
FLOAT MatICP::pythag(FLOAT a,FLOAT b) {
  FLOAT absa,absb;
  absa = fabs(a);
  absb = fabs(b);
  if (absa > absb)
