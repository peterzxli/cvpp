  ...
  for (int i=0, ii=0; i < num_cameras; i++)
    {
      ...
      msg.header.seq = i;
      msg.pose.position.x = nd.trans(0);
      msg.pose.position.y = nd.trans(1);
      msg.pose.position.z = nd.trans(2);
      msg.pose.orientation.w = nd.qrot.w();
      msg.pose.orientation.x = nd.qrot.x();
      msg.pose.orientation.y = nd.qrot.y();
      msg.pose.orientation.z = nd.qrot.z();
      pose_pub.publish( msg );

      std::cout << "######################################### Refined Cameras " << i << std::endl;
      std::cout << nd.trans << std::endl;
      std::cout << nd.qrot.w() << " " << nd.qrot.x() << " " << nd.qrot.y() << " " << nd.qrot.z() << std::endl;
