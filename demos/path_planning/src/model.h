...
////// AUXILIAR
    ...
    float cost( const String& type )
    {
        if( type == "DIST" ) return cost_distance( 0 , n() - 2 );
        if( type == "ACCEL" ) return cost_accel( 0 , n() - 2 );
    }
...
////// SHAKE
    ...
    float shake( const int& i , int n , const String& type = "DIST" )
    {
        ...
        if( type == "DIST" )
            return cost_distance( i , n );
        if( type == "ACCEL" )
            return cost_accel( i , n );
    }

////// COST

    float cost_distance( const int& i , const int& n )
    {
        if( !withinLimits() || !withinBounds() )
            return 1e12;
        ...
        {
            d += dist( Pt2f( states[j  ][0] , states[j  ][1] ) ,
...
////// MINIMIZE
    ...
    void minimize( const int& i , const int& n , const String& type )
    {
        if( i + n >= actions.n() ) return;
        Matf bmat = actions.mat().r(i,n).clone();
        float bval = shake( i , n , type );

        forLOOPk( 500 )
        {
            actions.mat().b(i,1,n,2).AddRand( 0.02 );
            float cval = shake( i , n , type );
    ...
    void minimize( const int& n , const String& type )
    {
...
////// DRAW
    ...
    void drawTrack( CPPlot& draw , const Vec5f& states ,
                    const Scalar& clr1 , const Scalar& clr2 , const int& siz )
    {
